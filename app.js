...
// 合约配置（请替换为你的实际合约地址）
let CONTRACT_ADDRESS = "0x7C54b41249e095d23B3E7392a9Bfc1aA0f186053";
// Ganache端口号：8545
const GANACHE_CHAIN_ID = 1337; // Ganache默认链ID
const CONTRACT_ABI = [
    ...
    },
    {
        ...
        "inputs": [
            ...
            { "indexed": false, "internalType": "uint8", "name": "diceNumber", "type": "uint8" },
            { "indexed": false, "internalType": "uint8", "name": "diceResult", "type": "uint8" }
        ],
        "name": "BetResult", "type": "event"
    },
    {
        "anonymous": false,
        "inputs": [
        ...
        "name": "ContractFunded", "type": "event"
    },
...
function isValidContractAddress(address) {
    // 以太坊地址验证规则：42个字符，以0x开头，其余为16进制字符
...
// ========== 监听BetResult事件（核心：获取骰子结果） ==========
// ========== 监听BetResult事件（增加调试日志） ==========
...
async function placeBet() {
    ...
    // 显示处理中状态
    showResult("⏳ 正在提交交易，请在MetaMask中确认...", "#ffd700");

    try {
        // 发送下注交易（适配Ganache 8545端口）
...
// ========== 连接MetaMask钱包（适配Ganache 8545） ==========
async function connectWallet() {
    if (typeof window.ethereum === 'undefined') {
        showResult("❌ 未检测到MetaMask，请先安装钱包插件", "#ff4444");
        if (confirm("未检测到MetaMask钱包，是否前往官网下载？")) {
    ...
    try {
        ...
        // 2. 检查并切换到正确的网络 (Ganache)
        const chainId = await web3.eth.getChainId();
        if (chainId !== GANACHE_CHAIN_ID) {
            showResult(`⚠️ 请切换到Ganache网络 (Chain ID: ${GANACHE_CHAIN_ID})`, "#ff9800");
            try {
                await window.ethereum.request({
                    method: 'wallet_switchEthereumChain',
                    params: [{ chainId: web3.utils.toHex(GANACHE_CHAIN_ID) }]
                });
            } catch (switchError) {
                if (switchError.code === 4902) {
                    showResult(`❌ 未添加Ganache网络，请手动添加 (RPC: http://127.0.0.1:8545)`, "#ff4444");
                } else {
                    throw switchError;
                }
            }
            return; // 切换网络后，让用户重新点击连接按钮
        }

        // 3. 初始化合约实例
        if (!CONTRACT_ADDRESS || !isValidContractAddress(CONTRACT_ADDRESS)) {
            showResult("❌ 合约地址无效！请在 app.js 中设置正确的地址。", "#ff4444");
            return;
        }
        guessDiceContract = new web3.eth.Contract(CONTRACT_ABI, CONTRACT_ADDRESS);
        ...
        window.ethereum.on('accountsChanged', async (newAccounts) => {
            ...
            if (accounts.length === 0) {
                ...
                document.getElementById("connectWalletBtn").textContent = "连接MetaMask钱包";
        ...
        window.ethereum.on('chainChanged', async (chainId) => {
            if (parseInt(chainId) !== GANACHE_CHAIN_ID) {
                showResult(`⚠️ 请切换到Ganache测试网络（8545，链ID: ${GANACHE_CHAIN_ID}）`, "#ff9800");
                document.getElementById("betBtn").disabled = true;
            } else {
                showResult("✅ 已切换到Ganache网络（8545）", "#4CAF50");
                document.getElementById("betBtn").disabled = false;
                await updatePageData();
            }
        });
...
window.onload = () => {
    ...
    // 初始化显示
    showResult("请先连接MetaMask钱包开始游戏", "#ffd700");

    // 【重要】请在这里设置你的合约地址
    // 示例：CONTRACT_ADDRESS = "0x你的合约地址";
    // 如需手动输入合约地址，可取消以下注释并在HTML中添加输入框
    /*
